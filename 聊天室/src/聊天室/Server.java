	...
	public void go(){
		...
		try{
			...
			while(true){
				...
				clientOutputStreams.put("Client"+clientOutputStreams.size(),w);
				//JOptionPane.showMessageDialog(null, "连接成功！");
				w.println(s.getInetAddress()+"加入群聊");
				w.println(clientOutputStreams.size());
				w.println("Client"+clientOutputStreams.size());
		 ...
		 public void tellEveryone(String message) {
		      ...
		      while(it.hasNext()) {
		        try {
		        	...
		        	PrintWriter writer = (PrintWriter) entry.getValue();//Java.io.PrintWriter 类打印格式化对象的表示到文本输出流。
